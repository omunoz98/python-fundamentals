...
colors = ['red', 'blue', 'green', 'purple', 'magenta', 'black', 'white',
          'orange']
...
# length and item validation, using len to get the length of a list
def simple_length():
...
def simple_search():
    if 98 in sizes:
        print('98 is in the list')
    else:
        print('98 is not in the list')
...
# del with no index removes list completely
def simple_del_none():
...
# remove items, removing items can be done in three ways: remove(), discard(),
# & pop(). remove will generate an error if the item being removed
# does not exist
...
# discard will remove without generating an error if the value
# is not in the list
...
# pop will remove the last item from the list so you don't know
# what it is
def simple_set_pop():
    letters = {'a', 'b', 'c', 'd', 'e', 'f'}
    letters.pop()
...
# joining sets collections by update, union and the | operator
even = {2, 4, 6, 8, 10}
odd = {1, 3, 5, 7, 9}
tens = {10, 20, 30, 40, 50}
...
# dictionary collection, are collections consisting of a key and
# value. the key is unique and can not be duplicate making immutable
# the value however can be a duplicate. Define using curly braces
